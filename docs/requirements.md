...
| D12 | Session `starts_at`/`ends_at` on write are **local wall-clock strings, no offset**, resolved server-side against the event's `timezone` (`utils.ResolveLocalTime`) — a spring-forward gap is rejected as 422, a fall-back ambiguity resolves to the earlier occurrence (a documented convention) | RFC3339 with an explicit offset, matching `events.starts_at`/`ends_at`'s existing shape (item 08) | Sessions are the precision-critical, DST-sensitive object the brief's failure scenario names ("someone misses the keynote"). Accepting a pre-computed instant deletes DST resolution from the server and scatters it across every client (agent, seed, tests), hiding exactly the class of bug this feature exists to catch. Cost: events and sessions now use two different time-input conventions on the same API — flagged for item 24's ADR candidates. Resolved via `AskUserQuestion` during item 12's plan, not assumed. |
| D13 | Inviting at a role above `attendee` is **admin-only** — a contributor may create an invitation only at `role=attendee`, enforced as an authorization decision (403 `cannot_invite_at_role`) inside `InvitationRepository.Create`'s atomic write, sharing item 09's `canGrantRoleActorCTE`/`canGrantRoleGuard` fragments verbatim (`internal/adapter/postgres/roles.go`) so member-add and invite cannot drift. The guard fails **closed**: an actor with no `event_members` row on the event at all is denied every invite, including `role=attendee` | leave it unenforced — `role_permissions` already grants `invitation:create` to both admin and contributor, and a bare permission check would let a contributor invite an admin | FEATURES.md/item 07's own contributor description ("invites attendees ... cannot change roles") states this boundary; `role_permissions` is action-level (create/read), not role-parameterized, so it cannot express "contributor may create at role=attendee only" on its own — the guard has to live in the write path, same as item 09's identical gap for `event_members`. Resolved via `AskUserQuestion` during item 13's plan review, not assumed. |
| D14 | Invitation `email` is visible to **both** admin and contributor on read (no role-gated field) — a **scoped exception** to D11, not a reopening of it | extend D11's admin-only-email rule uniformly to invitations too | D11 protects roster PII a contributor did not supply — an admin added those members, so a contributor reading them back would learn something they weren't given. An invitation's email is the opposite case: the contributor (who holds `invitation:create`) supplied that exact email to create the record: reading it back leaks nothing new. The honest gap: `invitations` has no `invited_by`/`created_by` column, so a contributor sees **every** invitation email on an event they have `invitation:read` for, not only ones they personally created — provenance-based scoping was rejected for D11 for the identical reason (no per-row authorship tracked) and is rejected here too, for consistency. See §8 for the "two more weeks" line. |
...
> **Session `room_id`/`speaker_id` are fixed at creation — no PATCH path for either.** `session.Patch` and the `SessionPatchRequest` OpenAPI schema have no fields for them at all, not merely fields left out of `required`. Rejected alternative: allow reassignment via PATCH, guarded by the same atomic "room belongs to this event" check `Create`'s CTE already does, extended into the version-gated `UPDATE`'s `WHERE` clause. Cut because no `must:` test needs it and the guard adds real complexity (a conditional `EXISTS` clause layered onto the versioned-update disambiguation logic) for a feature nobody asked for — CLAUDE.md: "Don't design for hypothetical future requirements." Two more weeks: add it back if a real workflow needs to move a session to a different room without deleting and recreating it, reusing `Create`'s exact room-validation CTE shape inside `Update`.

> **Invitation email visibility has no per-row provenance scoping (D14).** A contributor sees every invitation's email on an event they have `invitation:read` for, not only the ones they personally created — `invitations` carries no `invited_by`/`created_by` column. Rejected alternative: add that column and scope a contributor's view to their own invites, which would be the principled fix but costs a migration, a new authz-adjacent join, and a second presenter branch for a peripheral resource in a 15-hour budget. Chosen because the wider visibility leaks nothing a contributor didn't already type in themselves (D14's rationale) — it's a real gap, just a narrower one than D11's roster case. Two more weeks: add `invited_by uuid REFERENCES event_members(id)`, and scope `GET .../invitations`'s email field to rows the caller created (or is admin).
>
> **A wrong-role invitation cannot be corrected via the API once created (item 13).** `invitations` has no `version` column and no update/delete route — FEATURES.md scopes item 13 to "create and list" only, and the unique `(event_id, email)` constraint means even deleting and recreating isn't possible without going around the API (or the invitee changing their email). Rejected alternative: ship PATCH/DELETE now, ahead of any `must:` test needing them. Cut because nothing in FEATURES.md's item 13 scope or the brief asks for it, and item 21 (idempotent bulk invite) doesn't need it either — a bad invite is a low-cost mistake (no membership was granted) that a future feature can fix properly rather than bolting on now. Two more weeks: add a version column, a `DELETE`, and either a `PATCH` (role only, re-running the same escalation guard) or a documented "delete and re-invite" flow.
>
> **Session `starts_at`/`ends_at` on write are local wall-clock strings resolved server-side against the event's timezone, not RFC3339-with-offset like events (D12).** Rejected alternative: accept an instant with an explicit offset, matching `events.starts_at`/`ends_at`'s existing shape (item 08) — simpler, and consistent with the rest of the API. Rejected because sessions are the precision-critical, DST-sensitive object the brief's failure scenario ("someone misses the keynote because of a timezone bug") is actually about: accepting a pre-computed instant deletes the DST-resolution step from the server entirely and scatters it across every client (the agent, the seed generator, tests) instead, where a mistake stays silent rather than being caught by a single, tested `utils.ResolveLocalTime`. A local time that falls in a spring-forward gap is detected (round-trip mismatch) and rejected as 422, not silently normalized to a shifted instant. Cost: events and sessions now use two different time-input conventions on the same API, which is itself worth flagging in item 24's ADR candidates. Two more weeks: extend the same local+timezone convention to events' `starts_at`/`ends_at` for consistency, once there's a concrete need (events aren't currently DST-sensitive at minute granularity the way a session's keynote slot is).
